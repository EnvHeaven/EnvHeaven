...
function materializeDynamicVersionToken(value, artifactName, resolvedVersion) {
    const withFunctionTemplates = value.replace(DYNAMIC_ARTIFACT_VERSION_FUNCTION_PATTERN, (match, _quote, tokenArtifactName) => tokenArtifactName === artifactName ? resolvedVersion : match);
...
function materializeDynamicVersionExecution(execution, artifactName, resolvedVersion) {
    return {
        ...execution,
        args: execution.args.map((arg) => materializeDynamicVersionToken(arg, artifactName, resolvedVersion)),
