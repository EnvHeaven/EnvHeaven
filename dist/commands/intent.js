...
exports.inferCommandIntent = inferCommandIntent;
...
function inferCommandIntent(args) {
    ...
    for (const token of normalizedTokens) {
        if (HARD_REJECT_KEYWORDS.has(token)) {
            return {
                intent: null,
                diagnostics: [
                    (0, diagnostics_1.createDiagnostic)("error", "unsupported-command", `Unsupported token "${token}" in v0.1.0.`),
                ],
            };
        }
    }
    ...
    if (runCount > 1 || deployCount > 1 || offilineCount > 1) {
        return {
            intent: null,
            diagnostics: [(0, diagnostics_1.createDiagnostic)("error", "ambiguous-command", "Command contains repeated action tags.")],
        };
    }
    ...
    if (runCount === 0 && deployCount === 0) {
        ...
        if (normalizedTokens.length >= 1 && (normalizedTokens[0] === "daemon" || normalizedTokens[0] === "service")) {
            ...
            return {
                intent: null,
                diagnostics: [
                    (0, diagnostics_1.createDiagnostic)("error", "unsupported-command-shape", `Unsupported service subcommand: "${normalizedTokens.slice(1).join(" ")}". Valid: stop, restart, status.`),
                ],
            };
        }
        ...
        if (normalizedTokens.length >= 1 && normalizedTokens[0] === "ui") {
            ...
            return {
                intent: null,
                diagnostics: [
                    (0, diagnostics_1.createDiagnostic)("error", "unsupported-command-shape", `Unsupported ui subcommand: "${normalizedTokens.slice(1).join(" ")}". Valid: stop, restart, status.`),
                ],
            };
        }
        return {
            intent: null,
            diagnostics: [
                (0, diagnostics_1.createDiagnostic)("error", "unsupported-command", "Only the bare default target is accepted without the run tag in v0.1.0."),
            ],
        };
    }
    if (runCount === 1) {
        ...
        if (!parsedRun) {
            return {
                intent: null,
                diagnostics: [
                    (0, diagnostics_1.createDiagnostic)("error", "unsupported-command-shape", `Unsupported or ambiguous run target: "${targetTokens.join(" ")}".`),
                ],
            };
        }
        return {
    ...
    if (!parsedDeploy) {
        return {
            intent: null,
            diagnostics: [
                (0, diagnostics_1.createDiagnostic)("error", "unsupported-command-shape", `Unsupported or ambiguous deploy target: "${normalizedDeployTokens.join(" ")}".`),
            ],
        };
    }
    return {
...
function parseSupportedRunTarget(tokens) {
...
function parseSupportedDeployTarget(tokens) {
